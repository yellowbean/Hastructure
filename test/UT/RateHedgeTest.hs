...
capRateTests =
  let
    ...
    ctx = D.RunContext {}
    rc1 = accrueRC td2 ctx (Lib.toDate "20241231") indexAssump rc
  in
    ...
    [
      testCase "Accure out of scope" $
        assertEqual "before"
          (Right rc)
          (accrueRC td2 ctx (Lib.toDate "20231201") indexAssump rc)
      ,testCase "Accure out of scope" $
        assertEqual "after" 
          (Right rc)
          (accrueRC td2 ctx (Lib.toDate "20280101") indexAssump rc)
